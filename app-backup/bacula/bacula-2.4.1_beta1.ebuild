...
src_unpack() {
	...
	pushd src && epatch "${FILESDIR}/2.4.0/${PN}"-default-configs.patch && popd

	# replaces (deprecated) gnomesu with gksu in the gnome menu files
	useq bacula-console && useq gnome && epatch "${FILESDIR}/2.4.0/${PN}"-gnomesu2gksu.diff
}
...
#!		doins "${WORKDIR}/${PN}-docs-${DOC_VER}"/developers/developers/*
	...
	for script in ${myscripts}; do
		...
		cp "${FILESDIR}/2.4.0/${script}"-conf "${T}/${script}".conf
		cp "${FILESDIR}/2.4.0/${script}"-init "${T}/${script}".init
