...
SRC_URI="http://www.avenard.org/files/media/vdpau/mythtv-fixes-glvdpau20744.patch.gz"

IUSE_VIDEO_CARDS="video_cards_nvidia"
IUSE="aac alsa altivec autostart debug directv dvb dvd fftw ieee1394 jack lcd \
lirc mmx opengl perl python xvmc vdpau hdpvr ${IUSE_VIDEO_CARDS}"
...
src_unpack() {
	...
	epatch "${FILESDIR}"/${PN}-0.21-sandbox.patch

	if use hdpvr; then

		# Adding HDPVR support
		epatch "${FILESDIR}"/${PN}-0.21-hdpvr-unified.patch
		epatch "${FILESDIR}"/${PN}-0.21-hdpvr-DeviceReadBuffer-polltimeout.2.patch
		epatch "${FILESDIR}"/${PN}-0.21-hdpvr-cout-undeclared.patch

	fi

	if use vdpau; then 
		# Add VDPAU and OpenGL optimizations
		#epatch "${FILESDIR}"/${PN}-0.21-rev_20658-glvdpau.patch
		epatch "${DISTDIR}/mythtv-fixes-glvdpau20744.patch.gz"
		epatch "${FILESDIR}"/${PN}-0.21-audioencoding-fixes-vdpau-6.patch
	fi

	use aac && epatch "${FILESDIR}"/${PN}-0.21-latm-aac.patch
...
#	use aac && myconf="${myconf} --enable-libfaac"
	...
	if use hdpvr; then
		## MODIFIED BY MARSHALL FOR HDPVR, VDPAU, and OPENGL ENHANCEMENTS
		myconf="${myconf} --enable-hdpvr"
	fi

	if use vdpau; then
		myconf="${myconf} --enable-vdpau --enable-opengl-video"
	fi
