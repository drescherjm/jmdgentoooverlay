...
src_compile() {
	...
	local CMAKE_VARIABLES="-DCMAKE_SKIP_RPATH:BOOL=YES -DCMAKE_BUILD_TYPE:STRING=Release"
	...
	use jpeg && CMAKE_VARIABLES="${CMAKE_VARIABLES} -DGDCM_USE_SYSTEM_OPENJPEG:BOOL=ON"
	...
	if use test; then
	  	...
	  	CMAKE_VARIABLES="${CMAKE_VARIABLES} -DGDCM_BUILD_TESTING:BOOL=OFF"
	fi

	if use pdf; then
		CMAKE_VARIABLES="${CMAKE_VARIABLES} -DGDCM_PDF_DOCUMENTATION:BOOL=ON"
	else
		CMAKE_VARIABLES="${CMAKE_VARIABLES} -DGDCM_PDF_DOCUMENTATION:BOOL=OFF"
	fi
	...
	if use python; then
		...
		sed -e "s|/lib/python/site-packages|/lib/python$(python_get_version)/site-packages|" -i "${WORKDIR}/build/Wrapping/Python/cmake_install.cmake" \
		|| die "Failed to patch Python binding install path"

		# patch for vtkgdcm's binding is also needed
		if use vtk; then
			sed -e "s|/lib/python/site-packages|/lib/python$(python_get_version)/site-packages|" -i "${WORKDIR}/build/Utilities/VTK/cmake_install.cmake" \
